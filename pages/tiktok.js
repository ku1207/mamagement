...
export default function TiktokContent() {
  ...
  const generateTiktokData = () => {
    ...
    const adTypes = ['in_feed', 'spark_ads', 'brand_takeover', 'branded_hashtag', 'top_view']
    
    const campaigns = [
      '틱톡 브랜딩 캠페인', '틱톡 챌린지 캠페인', '틱톡 인플루언서 콜라보', '틱톡 바이럴 마케팅',
      '틱톡 댄스 챌린지', '틱톡 브랜드 해시태그', '틱톡 스파크 광고', '틱톡 인피드 광고'
    ]
    
    const adGroups = [
      '브랜드 해시태그', '트렌드 해시태그', '댄스 챌린지', '브랜드 챌린지',
      '인플루언서 콜라보', '바이럴 콘텐츠', '시즌 트렌드', '이벤트 해시태그'
    ]
  ...
  useEffect(() => {
    ...
    const loadTiktokData = async () => {
      ...
      try {
        ...
        const mockData = generateTiktokData()
        ...
        console.error('틱톡 데이터 로드 실패:', error)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <div className="content-area">
        <div className="no-advertiser-selected">
          <h2>📋 광고주를 선택해주세요</h2>
          <p>좌측 메뉴에서 광고주 계정을 선택하면 해당 광고주의 틱톡 광고 데이터를 확인할 수 있습니다.</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="content-area">
        <div className="loading-dashboard">
          ...
          <span className="loading-text">틱톡 광고 데이터 로딩 중...</span>
        </div>
      </div>
    )
  }
