...
export default function NaverPage() {
  const [currentPage, setCurrentPage] = useState('naver')
...
function NaverContent() {
  ...
  // 네이버 광고 가상 데이터 생성
  const generateNaverData = () => {
    const platform = 'naver'
    const adTypes = ['search', 'banner', 'display', 'shopping']
    
    const campaigns = [
      '네이버 브랜딩 캠페인', '네이버 세일 프로모션', '네이버 신제품 런칭', '네이버 리타겟팅 캠페인',
      '네이버 키워드 마케팅', '네이버 디스플레이 광고', '네이버 쇼핑 광고', '네이버 배너 광고'
    ]
  ...
  useEffect(() => {
    // 네이버 데이터 로드
    const loadNaverData = async () => {
      setLoading(true)
      
      try {
        // 가상 데이터 생성 (실제로는 API 호출)
        const mockData = generateNaverData()
        ...
        console.error('네이버 데이터 로드 실패:', error)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <div className="content-area">
        <div className="no-advertiser-selected">
          <h2>📋 광고주를 선택해주세요</h2>
          <p>좌측 메뉴에서 광고주 계정을 선택하면 해당 광고주의 네이버 광고 데이터를 확인할 수 있습니다.</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="content-area">
        <div className="loading-dashboard">
          ...
          <span className="loading-text">네이버 광고 데이터 로딩 중...</span>
        </div>
      </div>
    )
  }
