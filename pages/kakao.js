...
export default function KakaoPage() {
  const [currentPage, setCurrentPage] = useState('kakao')
...
function KakaoContent() {
  ...
  // 카카오 광고 가상 데이터 생성
  const generateKakaoData = () => {
    const platform = 'kakao'
    const adTypes = ['search', 'banner', 'display', 'moment']
    
    const campaigns = [
      '카카오 브랜딩 캠페인', '카카오 세일 프로모션', '카카오 신제품 런칭', '카카오 리타겟팅 캠페인',
      '카카오 키워드 마케팅', '카카오 디스플레이 광고', '카카오 모먼트 광고', '카카오 배너 광고'
    ]
  ...
  useEffect(() => {
    // 카카오 데이터 로드
    const loadKakaoData = async () => {
      setLoading(true)
      
      try {
        // 가상 데이터 생성 (실제로는 API 호출)
        const mockData = generateKakaoData()
        ...
        console.error('카카오 데이터 로드 실패:', error)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <div className="content-area">
        <div className="no-advertiser-selected">
          <h2>📋 광고주를 선택해주세요</h2>
          <p>좌측 메뉴에서 광고주 계정을 선택하면 해당 광고주의 카카오 광고 데이터를 확인할 수 있습니다.</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="content-area">
        <div className="loading-dashboard">
          ...
          <span className="loading-text">카카오 광고 데이터 로딩 중...</span>
        </div>
      </div>
    )
  }
