...
export default function GoogleContent() {
  ...
  const generateGoogleData = () => {
    ...
    const adTypes = ['search', 'display', 'video', 'shopping', 'app']
    
    const campaigns = [
      '구글 브랜딩 캠페인', '구글 세일 프로모션', '구글 신제품 런칭', '구글 리마케팅 캠페인',
      '구글 키워드 마케팅', '구글 디스플레이 광고', '구글 동영상 광고', '구글 쇼핑 광고'
    ]
  ...
  useEffect(() => {
    ...
    const loadGoogleData = async () => {
      ...
      try {
        ...
        const mockData = generateGoogleData()
        ...
        console.error('구글 데이터 로드 실패:', error)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <div className="content-area">
        <div className="no-advertiser-selected">
          <h2>📋 광고주를 선택해주세요</h2>
          <p>좌측 메뉴에서 광고주 계정을 선택하면 해당 광고주의 구글 광고 데이터를 확인할 수 있습니다.</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="content-area">
        <div className="loading-dashboard">
          ...
          <span className="loading-text">구글 광고 데이터 로딩 중...</span>
        </div>
      </div>
    )
  }
